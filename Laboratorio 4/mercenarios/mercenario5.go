...
func main() {
	...
	// muerto:
	sendMessage(fmt.Sprintf("%s:%s:%s", "M5", "1", strconv.Itoa(piso1)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M5", "2", strconv.Itoa(piso2)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M5", "3", elecciones))
}

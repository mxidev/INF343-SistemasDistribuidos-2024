...
func main() {
	...
	// muerto:
	sendMessage(fmt.Sprintf("%s:%s:%s", "M6", "1", strconv.Itoa(piso1)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M6", "2", strconv.Itoa(piso2)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M6", "3", elecciones))
}

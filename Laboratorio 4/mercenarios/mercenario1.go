...
func main() {
	...
	sendMessage(fmt.Sprintf("%s:%s:%s", "M1", "1", strconv.Itoa(piso1)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M1", "2", strconv.Itoa(piso2)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M1", "3", elecciones))
}

...
func main() {
	...
	// muerto:
	sendMessage(fmt.Sprintf("%s:%s:%s", "M3", "1", strconv.Itoa(piso1)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M3", "2", strconv.Itoa(piso2)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M3", "3", elecciones))
}

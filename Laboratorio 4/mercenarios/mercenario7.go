...
func main() {
	...
	// muerto:
	sendMessage(fmt.Sprintf("%s:%s:%s", "M7", "1", strconv.Itoa(piso1)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M7", "2", strconv.Itoa(piso2)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M7", "3", elecciones))
}

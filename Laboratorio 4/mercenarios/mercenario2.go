...
func main() {
	...
	// muerto:
	sendMessage(fmt.Sprintf("%s:%s:%s", "M2", "1", strconv.Itoa(piso1)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M2", "2", strconv.Itoa(piso2)))
	sendMessage(fmt.Sprintf("%s:%s:%s", "M2", "3", elecciones))
}
